...
export async function createWasmBackend(
  ...
  if (!factory) {
    // Check if already loaded as a global (e.g., via <script> tag)
    if (typeof (globalThis as any).createCbMpc === "function") {
      factory = (globalThis as any).createCbMpc;
    } else if (typeof document !== "undefined") {
      // Browser: load cbmpc.js as a classic script (it uses var, not export)
      const scriptUrl = new URL("../../cbmpc.js", import.meta.url).href;
      await new Promise<void>((resolve, reject) => {
        const script = document.createElement("script");
        script.src = scriptUrl;
        script.onload = () => resolve();
        script.onerror = () => reject(new Error("Failed to load cbmpc.js from " + scriptUrl));
        document.head.appendChild(script);
      });
      factory = (globalThis as any).createCbMpc;
    } else {
      // Node.js / Bun: use require() since cbmpc.js uses var (not export)
      // and "type": "module" makes import() treat .js as ESM
      const { createRequire } = await import("node:module");
      const _require = createRequire(import.meta.url);
      const glue = _require("../../cbmpc.js");
      factory = typeof glue === "function" ? glue : glue.default || glue.createCbMpc;
    }
  }
